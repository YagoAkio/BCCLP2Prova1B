...
export default function Candidato(props) {
    ...
    useEffect(() => {
        const descutidasSalvas = localStorage.getItem(`descurtidas_${props.candidato.nome}`)
        const curtidasSalvas = localStorage.getItem(`curtidas_${props.candidato.nome}`);
        const questionamentosSalvos = localStorage.getItem(`perguntas_${props.candidato.nome}`);
        if (curtidasSalvas) {
    ...
    const Curtir = () => {
        const novoNumeroCurtidas = curtidas + 1;
        setCurtidas(novoNumeroCurtidas);
        localStorage.setItem(`curtidas_${props.candidato.nome}`, novoNumeroCurtidas);
    };

    const NaoCurtir = () =>{
        const novoNumeroDescurtidas = descurtidas + 1;
        setDescurtidas(novoNumeroDescurtidas);
        localStorage.setItem(`descurtidas_${props.candidato.nome}`, novoNumeroDescurtidas); 
    }

    function detalhar (candidato){
        ...
        <Card style={{ width: '18rem' }}>
            ...
            <Card.Body>
                ...
                <Card.Text>
                    ...
                    <p>{"Questionamentos: " + questionamentos.length}</p>
                </Card.Text>
                <Button variant="primary" onClick={Curtir}>
                    {// Curtir
                    }
                ...
                <Button variant="primary" onClick={NaoCurtir}>
                    {// Não Curtir
                    }
                ...
                <Button className="mt-1" variant="success" onClick={() => {
                    detalhar(props.candidato);
                }} >
